...
var options = {
  ...
  form: {
  auth_token: token,
  body: 'Была заполнена форма пользователем '+params.name+'; Ссылка на страницу с формой: '+encodeURI(params.url)+';<br><br> Вся полученная информация: <br>'+params.allInfo 
  }
}
  request(options, function(err, res, body){
    if (err) {
